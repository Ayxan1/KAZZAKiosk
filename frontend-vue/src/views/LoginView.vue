  ...
  <div class="min-h-screen bg-gradient-to-br from-indigo-600 to-purple-700 flex items-center justify-center p-4">
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md p-8">
      ...
      <form @submit.prevent="handleLogin" class="space-y-6">
        <div>
          <label class="block text-sm font-medium text-gray-700 mb-2">İstifadəçi adı</label>
          <input 
            v-model="username"
            type="text" 
            required
        ...
        <div>
          <label class="block text-sm font-medium text-gray-700 mb-2">Şifrə</label>
          <input 
            v-model="password"
            type="password" 
            required
        ...
        <div v-if="showKioskSelect">
          <label class="block text-sm font-medium text-gray-700 mb-2">Kiosk</label>
          <select 
          ...
          >
            ...
            <option v-for="kiosk in kioskStore.kiosks" :key="kiosk.kiosk_id" :value="kiosk.kiosk_id">
              {{ kiosk.kiosk_name }}
            </option>
          </select>
        </div>
