<template>
  <div class="min-h-screen bg-gray-100">
    ...
    <header class="bg-white shadow-sm">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex justify-between items-center">
        <div>
          ...
          <p class="text-sm text-gray-600">Seller Panel - {{ authStore.user?.full_name }}</p>
        </div>
        <button 
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
      ...
      <div v-if="activeTab === 'sales'" class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            >
              <div class="flex justify-between">
                <div>
                  ...
                  <p class="text-sm text-gray-600">Stok: {{ product.stock_quantity }}</p>
                </div>
        ...
        <div class="bg-white rounded-xl shadow-sm p-6">
          ...
          <div class="space-y-3 mb-4">
            <div v-for="item in salesStore.cart" :key="item.product_id" class="flex justify-between items-center">
              <div class="flex-1">
                <p class="font-medium text-sm">{{ item.product_name }}</p>
                <div class="flex items-center gap-2 mt-1">
                  <button 
                    @click="salesStore.updateQuantity(item.product_id, item.quantity - 1)"
          ...
          <div class="border-t pt-4">
            <div class="flex justify-between text-xl font-bold mb-4">
              <span>Cəmi:</span>
              <span class="text-indigo-600">{{ salesStore.cartTotal.toFixed(2) }} ₼</span>
            </div>
      ...
      <!-- Inventory Tab -->
      <div v-else-if="activeTab === 'inventory'" class="bg-white rounded-xl shadow-sm p-6">
        ...
        <div class="space-y-2">
          <div v-for="product in productStore.products" :key="product.product_id" 
            class="p-4 border border-gray-200 rounded-lg"
          >
            <div class="flex justify-between">
              <div>
                <h3 class="font-medium">{{ product.product_name }}</h3>
                <p class="text-sm text-gray-600">Qiymət: {{ product.price }} ₼ | Stok: {{ product.stock_quantity }}</p>
              </div>
              <button class="text-indigo-600 hover:text-indigo-700">Redaktə</button>
            </div>
          </div>
        </div>
      </div>
      ...
      <!-- History Tab -->
      <div v-else-if="activeTab === 'history'" class="bg-white rounded-xl shadow-sm p-6">
    ...
    <!-- Add Product Modal -->
    <div v-if="showAddProduct" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4">
      <div class="bg-white rounded-xl p-6 max-w-md w-full">
        ...
        <form @submit.prevent="addProduct" class="space-y-4">
          ...
          <div class="flex gap-2">
            <button type="submit" class="flex-1 bg-indigo-600 text-white py-2 rounded-lg">Əlavə Et</button>
            <button type="button" @click="showAddProduct = false" class="flex-1 bg-gray-200 py-2 rounded-lg">Ləğv et</button>
          </div>
        </form>
      </div>
    </div>
